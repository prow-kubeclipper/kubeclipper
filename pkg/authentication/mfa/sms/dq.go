...
func (d dpSMSProvider) Verify(req url.Values, info user.Info) error {
	...
	if result.Code != 0 {
		return restful.NewError(http.StatusBadRequest,
			fmt.Sprintf("invalid sms code:%s", result.ErrorDescription))
	}
	return nil
}
...
func (d dpSMSProvider) Request(info user.Info) error {
	...
	case 0:
		return nil
	case 104, 105:
		return restful.NewError(http.StatusTooManyRequests, "SMS sending limit reached")
	default:
		return restful.NewError(http.StatusInternalServerError, fmt.Sprintf("send verification code:%s", result.ErrorDescription))
	}
}
