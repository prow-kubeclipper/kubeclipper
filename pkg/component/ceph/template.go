    ...
    spec:
      ...
      containers:
        - name: csi-provisioner
          image: {{with .ImageRepoMirror}}{{.}}/{{end}}caas4/csi-provisioner:v2.2.2
          args:
            - "--csi-address=$(ADDRESS)"
            - "--v=5"
            - "--timeout=150s"
        ...
        - name: csi-snapshotter
          image: {{with .ImageRepoMirror}}{{.}}/{{end}}caas4/csi-snapshotter:v4.1.1
          args:
            - "--csi-address=$(ADDRESS)"
            - "--v=5"
            - "--timeout=150s"
        ...
        - name: csi-attacher
          ...
          volumeMounts:
            - name: socket-dir
              mountPath: /csi
        - name: csi-resizer
          image: {{with .ImageRepoMirror}}{{.}}/{{end}}caas4/csi-resizer:v1.2.0
          args:
            - "--csi-address=$(ADDRESS)"
            - "--v=5"
            - "--timeout=150s"
        ...
        - name: csi-rbdplugin
          ...
          image: {{with .ImageRepoMirror}}{{.}}/{{end}}caas4/cephcsi:v3.4.0
          args:
            - "--nodeid=$(NODE_ID)"
            - "--type=rbd"
    ...
    spec:
      ...
      containers:
        - name: driver-registrar
          ...
          securityContext:
            privileged: true
          image: {{with .ImageRepoMirror}}{{.}}/{{end}}caas4/csi-node-driver-registrar:v2.2.0
          args:
            - "--v=5"
...
parameters:
  clusterID: {{.CSIConfig.CephClusterID}}
  pool: {{.PoolID}}
...
kind: StorageClass
metadata:
  name: {{.StorageClassName}}-qos
  annotations:
    storageclass.kubeclipper.io/rbd-qos: "use rbd-nbd as mounter to support qos,need kernel 5.4+"
provisioner: rbd.csi.ceph.com
parameters:
  clusterID: {{.CSIConfig.CephClusterID}}
  pool: {{.PoolID}}
