...
var (
  ...
  Please read 'kcctl deploy -h' get more deploy flags`, PkgURL, PkgURL, PkgURL, PkgURL)
)
...
func (d *DeployOptions) Complete() error {
	...
	if d.deployConfig.Config == "" {
		if d.deployConfig.Agents, err = deploy.BuildAgent(d.agents, d.fips, d.deployConfig.DefaultRegion); err != nil {
			return err
		}
	}
...
func (d *DeployOptions) confirm() bool {
	_, _ = d.IOStreams.Out.Write([]byte("Deploy will clear all data,please make sure you have backed up your data,still deploy? Please input (yes/no)"))
	return utils.AskForConfirmation()
}

func (d *DeployOptions) stopAll() {
	cmdList := []string{
		"systemctl stop kc-server||true",
		"systemctl stop kc-etcd||true",
		"rm -rf /var/lib/kc-etcd||true",
		"systemctl stop kc-agent||true",
		"systemctl stop kc-console||true",
		"rm -rf /etc/kc-console||true",
	}
	...
	err := sshutils.CmdBatchWithSudo(d.deployConfig.SSHConfig, d.deployConfig.ServerIPs, "systemctl stop kc-agent || true", sshutils.DefaultWalk)
	if err != nil {
		logger.Fatalf("deploy kc server failed due to %s", err.Error())
	}
...
func (d *DeployOptions) deployKcAgent() {
	for agent := range d.deployConfig.Agents {
		...
		cmdList := []string{
			...
			"systemctl daemon-reload", // don't start kc-agent
		}
		for _, cmd := range cmdList {
